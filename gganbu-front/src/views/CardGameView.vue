        ...
        card.addEventListener("click", function () {
          if (clickFlag && !completedOne.includes(card)) {
            ...
            if (clickedOne.length == 2) {
              //선택한 카드가 2개일때
              //색깔이 같으면 계속 오픈 시킴
              if (clickedOne[0].querySelector(".card-back").style.backgroundColor === clickedOne[1].querySelector(".card-back").style.backgroundColor) {
                console.log("matched!");
                completedOne.push(clickedOne[0]); //매치된 카드를 완성카드로 넣기
                completedOne.push(clickedOne[1]);
                clickedOne = []; //초기화
                if (completedOne.length === x * y) {
                  //모두 매치했을 경우 성공 메시지
